...
> **Second game in this project — ORBIT SNAKE.** Play `Assets/Scenes/OrbitSnake.unity` (or `open Builds/ORBIT.app`). A/D or ←/→ turn, Space ejects the train, Esc pauses, Enter restarts. Top-down over a small planet; you walk a shell at constant speed and junk rides great circles of the same shell. Come up **behind** a piece so your heading matches its motion (it turns green when it will) and it latches on as a segment. Meet it any other way (red) and you lose the two hindmost segments; with none left, you're done. Touch your own tail past the neck and it severs — the loose segments become wreckage on this shell and come round again. When the pips at the bottom fill (quota 4 → 6 → 8 → 10) a green ring breathes around the ship: Space throws the train into the atmosphere for `n²·5 + 50·level` points and lifts you one shell into a denser field. Each climb floats three skill icons ahead of you; fly through one to take it (magnet: wider catch cone · armour: one free strike · whip: Q fires the last segment · brake: hold S · phase: double-tap A/D to hop sideways · compound: ejections pay 1.5×). Five shells; the fifth is escape. Stay too long on one shell and the Kessler clock adds junk every six seconds (the current shell's arc goes orange); die and your train stays on that shell as gold wreckage for the next run. **The HUD has no words**: five arcs for the shell, pips for the train, glyphs for skills, digits for the score and, smaller, your best. Any key starts; Esc pauses; Enter or Space restarts. Gamepad: left stick turns, A ejects, X whips, LT brakes, LB/RB phase-hop, Start pauses. Music and every sound are synthesised at start-up: the loop gains a layer and steps up a whole tone with each shell, and consecutive catches climb the scale. Design notes in `docs/ORBIT-SNAKE-DESIGN.md`; headless checks via `-executeMethod OrbitHeadlessRunner.Run` (24), screenshots via `OrbitScreenshotRunner.Run`, build via `RiftBuild.OrbitMacOS`.
...
The scope at bottom right is the aircraft's own frame at 14 m per pixel: refinery rings are hollow blips in their owner colour (the overcharged one breathes violet), rivals are orange dots, the ace gold, the vendetta target red with a breathing ring, and anything past 965 m is pinned to the rim so its bearing survives.
