...
public partial class OrbitSnake
{
    ...
    AudioSource sfxSrc, catchSrc, musicA, musicB, tensionSrc; bool musicOnA=true; float musicFade=1, catchStep, catchChainTimer; int catchChain;
    AudioClip[] loops=new AudioClip[5]; AudioClip tension; AudioClip[] fx=new AudioClip[9]; int musicLevel=-1;
    ...
    void BuildAudio()
    {
        ...
        sfxSrc=Src(.8f,false); catchSrc=Src(.7f,false); musicA=Src(0,true); musicB=Src(0,true); tensionSrc=Src(0,true);
        ...
        for(int i=0;i<loops.Length;i++)loops[i]=MusicLoop(Mathf.Min(i,3)); tension=TensionLoop();
        tensionSrc.clip=tension; tensionSrc.Play();
    }
    // Ping kinds: 0 strike, 1 catch, 2 shot/armour, 3 win, 4 sever, 5 eject, 6 skill, 7 start, 8 death.
    public void Ping(int kind)
    {
        ...
        sfxSrc.PlayOneShot(fx[kind],kind==3||kind==7?.9f:.75f);
    }
    ...
    public AudioClip LoopClip(int i) => loops[Mathf.Clamp(i,0,loops.Length-1)]; public AudioClip TensionClip => tension; public int FxCount => fx.Length; public float CatchPitch => catchSrc?catchSrc.pitch:1;

    // Per frame on the unscaled clock: pick the loop for the shell, cross-fade, follow pause and the Kessler drone.
