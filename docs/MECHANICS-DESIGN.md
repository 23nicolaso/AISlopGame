...
## 13. Salvage Chain 与 Refinery Heist —— 给"日常动作"一个节奏

**Chain**：捡货一局发生几十次，反馈层按"routine action"只给 4 层，但它可以有*节奏*：3 s 内连续拾取让倍率 ×1 → ×1.25 → ×1.5 → ×1.75 → ×2（第五块起封顶）。一片散落的货物是一条最优路线，而不是一堆等着被绕回去捡的零头。对 AI 同样生效，所以不改变玩家与 AI 的相对经济，只改变"怎么捡"。HUD：CARGO 右侧金色 `CHAIN x1.5` + 3 s 泄放条。死亡、重开清链。

**Heist**：从活着的对手手里夺环，占领 +25 之外再 +15，事件流双向播报（`HEIST +15 FROM …` / `REFINERY n TAKEN BY …`）。目的和 Vendetta 一样：让排行榜上的名字变成具体的人。

**验证**：三块 8 值货物 → 8/10/12；3.1 s 后回到 ×1；从活着的 Moth.exe 手里夺环 = 25 + 15。

## 12. Vendetta（私仇）—— 死亡之后的下一件事

**问题**：被击落后重生的三秒是整局最空的时刻——货物没了、目标没了、HUD 上只有一个倒计时。fps-shooter 的经验是"死亡必须立刻给出一个新目标"。

**机制**：击落玩家的凶手被标记 60 s。窗口内亲手打下它，它当时携带的货物作为奖励直接记入 banked（最低 20）；被别人抢走或超时就失效，且永远只有一个目标（新的死亡覆盖旧的）。AI 已经有 `revenge` 参数，这条让玩家拥有同样的东西，但以奖励而不是脚本的形式。

**数字**：窗口 60 s（一次追击 + 一次拉开的长度）；奖励 = max(20, 对方 cargo)——空手的凶手也值得追，满载的凶手是双倍收益。

**验证**：`RiftCombatVerification` 三项——标记建立、结算加分并清除、超时清除。
