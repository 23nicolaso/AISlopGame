...
## 11. 平衡跑测告诉我的（2026-09-21）

`Assets/Editor/RiftBalanceRunner.cs` 让 AI 坐满八个座位打整场比赛。基线（所有机制落地时）每局 0–2 杀、14–16 次自毁、多数人 0 分——机制全在，但没人活着用它们。十二轮之后（`docs/BALANCE-REPORT.md`）自毁降到 1–2 次、首杀 5–7 s、存分 2–9 次。三条会影响后续设计的结论：

- **稀薄空气是一门炮。** 400 m 以上没有阻力，任何抬头都把速度一比一换成高度，而且不会自己泄掉。§9 的再入过热和低轨站点都建立在这上面，但 AI（和玩家）需要"能量规则"：当前爬升率关引擎后还能再爬的高度不能超过目标高度 40 m。
- **一颗星球对八架飞机太大。** 950 m 视距、170 m 高度的地平线只有 640 m，环绕整颗星球的六个站点让 77% 的比赛时间花在赶路。站点现在是 32° 一个的 160° 弧，星球背面留给亚轨道和月亮。
- **转弯半径决定存分。** 100 m/s 在厚空气里的转弯半径 240 m、在 460 m 站点 640 m，105 m 的环从圈内是飞不进去的——AI 现在先拉开 600 m 再一次转弯直线穿环；对玩家这意味着低轨站点的存分本身就是一个技巧动作。
